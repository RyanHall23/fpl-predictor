...
const getRecommendedTransfers = async (req, res) => {
  ...
  try {
    ...
    // Fallback: use current price as purchase price for any players without a known purchase price
    if (Object.keys(purchasePriceMap).length === 0) {
      ...
      console.log(`[getRecommendedTransfers] Using current prices as fallback for ${Object.keys(purchasePriceMap).length} players`);
    }
