...
const App = () => {
  ...
  return (
    <Box sx={ { minHeight: '100vh', backgroundColor: theme.palette.background.default, display: 'flex', flexDirection: 'row' } }>
      ...
      >
        { /* ── Desktop Bento Grid — visible on lg+ ── */ }
        <Box
          sx={ {
            display: { xs: 'none', lg: 'grid' },
            gridTemplateAreas: `
              "pitch     pitch     fixtures  teamstats  rectransfers"
              "pitch     pitch     league    recentperf rectransfers"
              "statspod  controls  league    assistant  planned"
            `,
        ...
        >
          { /* ── pitch tile — spans rows 1-2, cols 1-2 ── */ }
          <Box sx={ { gridArea: 'pitch', overflow: 'hidden', display: 'flex', flexDirection: 'column' } }>
            { viewingOpponentId && (
              <Box sx={ { mb: 0.5, display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 } }>
          ...
          { /* ── statspod tile — row 3, col 1: compact stats ── */ }
          <Paper sx={ { gridArea: 'statspod', overflow: 'hidden', p: 1.5 } }>
            <Box sx={ { display: 'grid', gridTemplateColumns: `1fr 1fr${ displayFreeTransfers != null ? ' 1fr' : '' }${ displayBank != null ? ' 1fr' : '' }`, textAlign: 'center', rowGap: 0.5 } }>
          ...
          { /* ── controls tile — row 3, col 2: chips + auto pick + view toggle ── */ }
          <Paper sx={ { gridArea: 'controls', overflow: 'hidden', p: 1.5, display: 'flex', alignItems: 'center', gap: 1.5, flexWrap: 'wrap' } }>
            { !isHighestPredictedTeam && !viewingOpponentId && !isLockedGameweek && activePlayers.length > 0 && unusedChipIds.length > 0 && (
              <Box sx={ { display: 'flex', flexDirection: 'column', gap: 0.5 } }>
                <Typography variant='caption' color='text.secondary' sx={ { fontWeight: 500 } }>Chips</Typography>
          ...
          { /* ── fixtures tile — row 1, col 3 ── */ }
          <Paper sx={ { gridArea: 'fixtures', display: 'flex', flexDirection: 'column', overflow: 'hidden', p: 1.5 } }>
            <FixturesPanel
              gameweek={ selectedGameweek || currentGameweek }
              deadline={ gameweekInfo?.data?.deadline_time }
              liveMatches={ liveMatches }
            />
          </Paper>

          { /* ── league tile — rows 2-3, col 3 ── */ }
          <Box sx={ { gridArea: 'league', overflow: 'hidden', display: 'flex', flexDirection: 'column' } }>
            <RightPanel
              section='league'
          ...
          { /* ── teamstats tile — row 1, col 4 ── */ }
          <Paper sx={ { gridArea: 'teamstats', overflow: 'hidden', p: 1.5 } }>
            <TeamActivityPanel
              section='teamstats'
              entryId={ viewingOpponentId || currentEntryId }
              currentGameweek={ currentGameweek }
          ...
          { /* ── recentperf tile — row 2, col 4 ── */ }
          <Paper sx={ { gridArea: 'recentperf', overflow: 'hidden', p: 1.5 } }>
            <TeamActivityPanel
              section='recentperf'
              entryId={ viewingOpponentId || currentEntryId }
              currentGameweek={ currentGameweek }
          ...
          { /* ── assistant tile — row 3, col 4 ── */ }
          <Paper sx={ { gridArea: 'assistant', overflow: 'hidden', p: 1.5 } }>
            <TeamActivityPanel
              section='assistant'
              entryId={ viewingOpponentId || currentEntryId }
              currentGameweek={ currentGameweek }
          ...
          { /* ── rectransfers tile — rows 1-2, col 5 ── */ }
          <Paper sx={ { gridArea: 'rectransfers', display: 'flex', flexDirection: 'column', overflow: 'hidden', p: 1.5 } }>
            <TeamActivityPanel
              section='rectransfers'
              entryId={ viewingOpponentId || currentEntryId }
              currentGameweek={ currentGameweek }
              currentEntryId={ currentEntryId }
              viewingOpponentId={ viewingOpponentId }
          ...
          { /* ── planned tile — row 3, col 5 ── */ }
          <Paper sx={ { gridArea: 'planned', overflow: 'hidden', p: 1.5 } }>
            <TeamActivityPanel
              section='planned'
              entryId={ viewingOpponentId || currentEntryId }
              currentGameweek={ currentGameweek }
        ...
        { /* ── Mobile layout — visible on xs/sm/md only ── */ }
        <Box sx={ { display: { xs: 'flex', lg: 'none' }, flexDirection: 'column', flex: 1, px: { xs: 1, sm: 2 }, py: 1 } }>
          { /* Tab 0 — My Team */ }
          { mobileTab === 0 && (
